...
class Add_Customer_Page:
    ...
    newsletter_cusrole_list_xpath = "//ul[@class='select2-selection__rendered']//input[@type='search']"
    #* It has 2 elements *
    ...
    chbx_must_change_password_id = "MustChangePassword"
    ...
    def newsletter(self,value):
        ...
        if value == "Your store name":
            self.driver.find_element(By.XPATH,"//l1[contains(text(),'Your store name')]").click()
        elif value == "Test store":
            self.driver.find_element(By.XPATH,"//li[contains(text(),'Test store')]").click()
        else:
    ...
    def select_manager_of_vendor(self):
        drp_dwn = Select(self.driver.find_element(By.ID,self.drpdwn_mngrofvendor_id))
        drp_dwn.select_by_index(1)
        time.sleep(3)

    def customer_save_password(self):
        self.driver.find_element(By.ID,self.chbx_must_change_password_id).click()

    def enter_admin_comments(self,value):
        self.driver.find_element(By.ID,self.text_admincomment_id).send_keys(value)
